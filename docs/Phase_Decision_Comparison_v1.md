...
| **Yacoubi et al. (2025, AIAI)** [#03] | XGBoost + SHAP-driven k=15 feature selection → **99.80 % acc / 95.41 % macro-F1** (Table 1), but **6-class (six categories)**, raw data | E7 on the **harder 19-class** deduped, macro-F1 0.9076 | **Coarser task** (6 categories, not 19 sub-types) on raw data — **not a like-for-like comparison**: we do **not** set their 6-class F1 95.41 % against our 19-class macro-F1 0.9076 (more classes = harder, the same non-comparability as Abo-Haat's device-disjoint axis). The 99.80 % accuracy is at a granularity where minority sub-types are merged away. |
...
| **Weak spots (stated up front)** | — | **22.9 % fusion-level benign FPR** at entropy_benign_p95; **Recon_VulScan** is the marginal eligible target — **three distinct, labeled metrics**: AE-only **strict zero-day recall 0.441**, **entropy-gate rescue rate 0.745** (barely clears the 0.70 strict bar), and **binary recall 0.649 at p95** — passes at p90 (0.700 exactly) but **fails at p95**, "the system's weakest point" (README §15.4 / §15.6) | The 4/4 pass is **bounded above by 4** (MQTT_DoS_Connect_Flood structurally excluded, `n_loo_benign = 0`), carries a **high operational FPR**, and Recon_VulScan sits **on the margin** (and outright fails binary recall at p95). None of this is hidden. |
...
| **Yacoubi et al. (2025, COCIA)** [#02] | RF + CatBoost, **global SHAP + LIME** → RF **99.92 % (binary accuracy, raw data)**, **ROC-AUC 100.00 %** | per-class SHAP on deduplicated 19-class | **Global-scope only**, no per-class breakdown. Their 99.92 % is **binary accuracy on raw (non-deduplicated) data**, and the **ROC-AUC of 100.00 % is itself a leakage tell** — not a like-for-like against our 19-class per-class analysis. Overlap on "uses SHAP," divergent on granularity. |
