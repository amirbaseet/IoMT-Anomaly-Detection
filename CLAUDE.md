...
0. ~~Gap-doc + v6.6 integration~~ **DONE 2026-07-29**; ~~advisor format confirmation + §2.6 refs~~ **DONE 2026-07-30**; ~~thesis full draft (5 chapters)~~ **DONE 2026-07-30** (top entry). Hoca delivery is now ONE batch per A2: gap report + summary + full thesis draft + P1–P3/defense questions — gated on §4.5 fill (tau-sweep), the 18/19 reconciliation, and the memorization-premium doc sweep.
1. Fold `results/tausweep/tau_sweep_summary.md` into the thesis generalization section — **now also fills manuscript §4.5 (A3 stub)**; needs the user's uncommitted tau-sweep files committed first.
1b. ~~Reconcile the 18-vs-19 eligible-cell accounting~~ **RESOLVED 2026-07-30: artifact says 18** (`multi_seed_per_target_summary.csv` n_seeds 5+0+5+3+5); numbers_map/pathB/full_report/decisions_ledger/AUDIT/lit-review/manuscript all corrected to **0/18**; TUTARLILIK NOTU cleared. Residual: README (L1272/1306/1347/1500) + PJ (L453 has a third wrong accounting "19=20−1") still say 19 — fix rides with the user's pending README/PJ commits (queue 4).
1c. **Memorization-premium doc sweep** (post-C1-multiseed, joint with the P1 arc): align gap report §3 (L74), Research_Gap_Summary L14, and thesis Ch1 §1.1 with the retired claim (test-set effect +0.35–0.40pp is the separable finding); v6.6 needs only a check (its §2.2.8 "cost" framing predates and likely survives); Ch5 already cites the separable result.
